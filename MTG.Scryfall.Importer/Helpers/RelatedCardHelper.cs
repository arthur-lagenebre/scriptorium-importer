    ...
    public static List<RelatedCard> CreateRelatedCards(List<ScryfallAllPart> scryfallAllParts, string? scryfallId)
    {
        ...
        foreach (var part in scryfallAllParts)
            if (part.Id != scryfallId)
                relatedCards.Add(new RelatedCard(EnumHelper.GetRelatedCardComponent(part.Component), StringHelper.GetDefaultValue(part.Name), StringHelper.GetDefaultValue(part.TypeLine)));
